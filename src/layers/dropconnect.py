...
class WeightDropLSTM(torch.nn.LSTM):
    ...
    def __init__(self, *args, dropout=0.3, **kwargs):
        ...
        _weight_drop(self, weights, dropout)

    def forward(self, x):
        self.flatten_parameters() # Need to flatten parameters because RNN module weights are not part of single contiguous chunk of memory. This means they need to be compacted at every call, possibly greatly increasing memory usage. To compact weights again call flatten_parameters().
...
if __name__ == "__main__":
    ...
    weights = ['weight_hh']
    weight_drop_lstm = WeightDropLSTM(2,2, dropout=.0)
    weight_drop_linear = WeightDropLinear(2,1, dropout=0.)
    input_ = torch.randn(2, 2)
    out, states = weight_drop_lstm(input_)
    print(out[:, -1])
    out = weight_drop_linear(out[:, -1])
