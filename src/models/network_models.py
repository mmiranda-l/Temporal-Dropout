...
from .single.encoders import MLP, RNNet, BayesianEncoders
...
def create_model(input_dims, emb_dims: int, model_type: str = "mlp", n_layers: int = 2, batchnorm=False, dropout=0, use_norm_emb=False, encoder=True, uncertainty_estimation=True, **args):
    ...
    elif approx_type == None and (model_type =="rnn" or model_type== "gru" or model_type== "lstm"):
        print("Initializing basic encoder")
    ...
    elif model_type == "tempcnn": #performns better
    ...
    elif approx_type.lower() in ["dropconnect", "dropout", "concretedropout"]:
        args["num_layers"] = n_layers
        print("Initializing Generic Bayesian Encoder")
        sub_model = BayesianEncoders(**args)
    ...
    if encoder:
        ...
        return Generic_Bayesian_Decoder(decoder=sub_model, out_dims=emb_dims, use_norm= use_norm_emb, mc_samples=mc_samples, **args)
    else:
        return Generic_Decoder(decoder=sub_model, out_dims=emb_dims, use_norm= use_norm_emb, **args)
